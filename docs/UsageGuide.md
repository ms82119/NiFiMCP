...
3. **Named path (flow-as-code)** – Use `export_flow_to_path` to write a process group’s flow definition to an exact filesystem path (e.g. `flows/uat/my-pg/flow.json`) for git. Omit `process_group_id` to export the root group.
4. **Import / replace** – `import_flow_definition` uploads a JSON file as a **new child** process group under a parent (NiFi upload API). `replace_process_group_from_file` **replaces** an existing process group’s contents (async replace-requests; waits until complete). Prefer sandbox PGs; replace is destructive to the target group’s current contents.
5. **Structural diff** – `diff_flow_json` compares two flow JSON files without calling NiFi. By default it ignores layout noise (positions, style, `propertyDescriptors`, `instanceIdentifier`) and reports added/removed/changed processors, connections, services, and nested groups.
